...
class JaxArtifactsCapability:
    contract = BackendCapabilityContract(
        backend="jax",
        capability="artifacts",
        provides=("artifact.parameters.summary", "artifact.jax_model.describe"),
        methods={
            "artifact.parameters.summary": "parameter_layout_summary(model) -> dict",
            "artifact.jax_model.describe": "describe_model(model) -> dict",
        },
    ...
    def parameter_layout_summary(self, model: Any) -> dict[str, Any]:
        if not hasattr(model, "parameter_shapes"):
            raise TypeError("jax artifact summary requires model.parameter_shapes()")
...
__all__ = ["JaxArtifactsCapability"]
