...
| 路径 | 作用 |
|---|---|
| `run_solver.py` | Case-local debug entry for the ETF walk-forward runner. |
| `build_solver.py` | canonical Case assembly entry; returns the ETF walk-forward runner. |
| `config.py` | case-level component registry aggregation. |
| `problem/` | ETF problem layer. |
| `pipeline/` | ETF feature construction layer. |
| `plugins/` | report / observability capability layer. |
| `mlblack.integrations.etf_temporal_forecast` | stable integration surface used by the case implementation. |
...
Project formal entry is `python examples\cases\etf_temporal_forecast\run_project.py --check --build-check`; case-local `run_solver.py` is the direct debug entry.
...
一次 quickstart / serious 运行后，`runs/etf_temporal_forecast/` 会生成：
