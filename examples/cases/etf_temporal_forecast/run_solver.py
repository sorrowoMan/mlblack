...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if bool(args.check):
        print(
            "etf_temporal_forecast scaffold ok | "
            f"dataset={Path(cfg.dataset_url).expanduser().resolve()} models={cfg.models}"
        )
        return
    ...
    agg = dict(result.summary.get("aggregate", {}) or {})
    data = dict(result.summary.get("dataset", {}) or {})
    print(f"[etf-temporal] output_dir={result.output_dir}")
    print(
        "[etf-temporal] dataset="
        f"{data.get('rows')}x{data.get('assets')} {data.get('start')}..{data.get('end')}"
    )
    print(f"[etf-temporal] fold_count={result.summary.get('fold_count')} models={cfg.models}")
