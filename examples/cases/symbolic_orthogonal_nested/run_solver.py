...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if bool(args.check):
        print(
            "symbolic_orthogonal_nested scaffold ok | "
            f"stage1_dim={stage1_solver.problem.dimension} "
            f"stage1_objectives={stage1_solver.problem.objectives} "
            f"stage1_pool_terms={len(stage1_solver.problem.function_pool.terms)} "
            f"resource_context={dict(cfg.resource_context)}"
        )
        return
