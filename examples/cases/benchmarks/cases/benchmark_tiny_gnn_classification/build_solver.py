...
def build_solver(
    ...
    *,
    ...
    trainer_builder = overrides.pop("trainer", build_tiny_gnn_graph_classification_trainer)
    max_steps = overrides.pop("max_steps", None)
    if overrides:
        raise ValueError("unsupported benchmark_tiny_gnn_classification overrides: " + str(sorted(overrides)))
    data = data_builder() if callable(data_builder) else data_builder
    trainer = trainer_builder(
        data,
        hidden_dim=6, num_layers=2, learning_rate=1e-2, random_seed=53,
        run_name="benchmark_tiny_gnn_classification",
    ...
    if max_steps is not None:
        trainer.register_controller(
            BudgetController(
                ...
                name="benchmark_tiny_gnn_classification.step_limit",
            )
        )
    return trainer
