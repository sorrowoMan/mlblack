...
from .path_defaults import default_work_ci_csv


@dataclass
class WorkCiIntervalReader:
    """Reader for the traffic Work-CI interval dataset table."""
    ...
    def read(self) -> TrainDataBundle:
        ...
        train_ds = ProcessedDataset(
            ...
            metadata={"source": self.csv_path, "split": "train", "split_rule": f"{self.test_fold_col}==0"},
        )
        test_ds = ProcessedDataset(
            ...
            feature_names=tuple(feature_cols),
            target_names=(self.target_col,),
            metadata={"source": self.csv_path, "split": "test", "split_rule": f"{self.test_fold_col}==1"},
        )
