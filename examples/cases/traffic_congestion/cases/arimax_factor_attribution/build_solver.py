...
def build_solver(config=None, *, resource_context=None, component_overrides=None):
    ...
    return build_diagnostic_solver(
        runner,
        name="traffic_arimax_factor_attribution",
...
def _run_analysis(config=None):
    ...
    args = SimpleNamespace(
        ar_order=int(payload.get("ar_order", 2)),
        ma_order=int(payload.get("ma_order", 1)),
        diff=int(payload.get("diff", 0)),
    )

    pipeline_data = build_pipeline(CSV_PATH, factor_groups=FACTOR_GROUPS)
    available_groups = pipeline_data.factor_groups
    X_scaled = pipeline_data.X_scaled
    y = pipeline_data.y

    print("=" * 60)
    print("ARIMAX Factor Attribution: Traffic CI")
    print(f"Samples: {len(y)}, External features: {X_scaled.shape[1]}")
