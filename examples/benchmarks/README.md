...
```powershell
python examples\benchmarks\symbolic_orthogonal_nested_benchmark.py --check
```

Small run:

```powershell
python examples\benchmarks\symbolic_orthogonal_nested_benchmark.py --variants point,interval --generations 1 --pop-size 4 --inner-steps 3
```
...
```powershell
python examples\benchmarks\neural_graph_benchmark_matrix.py --steps 2 --repeats 3
```
