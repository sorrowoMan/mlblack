...
from nowcasting_work_ci.mlblack_side.problem.domain_router import build_work_ci_conditional_router_policy
from nowcasting_work_ci.mlblack_side.problem.problem_model import SymbolicSubsetSelectionProblem
from plugins.report_writer import write_summary_report


def _match_threshold_multiplier_feature(feature_name: str, feature_names: Sequence[str]) -> str | None:
...
def _build_default_threshold_specs(feature_names: Sequence[str]) -> dict[str, AutoThresholdBindingSpec]:
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--graph-cache-namespace",
        type=str,
        default=MlblackRuntimeConfig().graph_cache_namespace,
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.03,
        help="Final selection tolerance on coverage_error before prioritizing PINAW/Interval Score.",
    )
    ...
    parser.add_argument(
        "--temporal-pack-cross-quantiles",
        type=str,
        default="0.5",
        help="Comma-separated quantiles for targeted ci_lag1 hinge crosses in temporal pack.",
    )
    ...
    temporal_pack_added_features: list[str] = []
    temporal_pack_rolling_added: list[str] = []
    temporal_pack_momentum_added: list[str] = []
    temporal_pack_cross_added: list[str] = []
    temporal_pack_ratio_added: list[str] = []
    ...
    lag_cross_q = [float(np.clip(v, 0.01, 0.99)) for v in parse_float_list_csv(str(args.lag_cross_quantiles), default=(0.25, 0.5, 0.75))]
    ...
    if temporal_pack_enabled:
        ...
        X_train = np.asarray(temporal_pack.X_train, dtype=float)
        X_test = np.asarray(temporal_pack.X_test, dtype=float)
        feature_names = tuple(str(v) for v in temporal_pack.feature_names)
    ...
    gate_names = WORK_CI_STRICT4_GATE_NAMES
    ...
    router_policy = build_work_ci_conditional_router_policy() if strict4_enabled else None
    router_features = (
        tuple(str(v) for v in router_policy.gate_names if str(v) in feature_names)
    ...
    threshold_features = tuple(
        ...
        if str(name).startswith("ci_lag") or str(name).startswith("shock_") or str(name).startswith("volatility_")
    )
    conditional_feature_roles = build_feature_role_config(
        feature_names,
        router_features=router_features,
        threshold_features=threshold_features,
        metadata={
            "source": "runner_runtime_auto_roles",
            "strict4_enabled": bool(strict4_enabled),
        },
    )
    conditional_threshold_bindings = build_auto_threshold_bindings(
        X_train,
        feature_names,
        threshold_features=threshold_features,
        per_feature_specs=_build_default_threshold_specs(feature_names),
    )
    conditional_config = ConditionalConfig(
        enabled=True,
        feature_roles=conditional_feature_roles,
        router_policy=router_policy,
        binary_gates=tuple(BinaryGateBinding(feature_name=name) for name in router_features),
        threshold_primitives=tuple(conditional_threshold_bindings),
        composer=ConditionalComposerConfig(
            mode="route_then_formula" if router_policy is not None else "formula_only_primitives",
        ),
        metadata={
            "source": "runner_runtime_defaults",
            ...
            "threshold_binding_count": int(len(conditional_threshold_bindings)),
        },
    )
    ...
    def _run_outer_once(
        *,
        ...
        problem_local = SymbolicSubsetSelectionProblem(
            ...
            regime_branch_mode=bool(strict4_enabled),
            regime_gate_idx=strict4_gate_idx,
            regime_min_branch_train=int(max(8, args.strict4_min_branch_train)),
            regime_branch_parallel_workers=int(effective_strict4_workers),
        ...
        solver_local, outer_meta_local, _adapter_local = build_nowcasting_solver(
            ...
            eval_batch_proxy_fn=problem_local.evaluate_population_batch if bool(batched_eval_enabled) else None,
        )
    ...
    def _row_objective_key(row: Mapping[str, Any]) -> tuple[float, float, float]:
        cov = float(row.get("obj_coverage_error", float("inf")))
        pin = float(row.get("obj_pinaw", float("inf")))
        iscore = float(row.get("obj_interval_score", float("inf")))
        thr = float(max(0.0, args.selection_coverage_error_threshold))
        if cov <= thr:
    ...
    for ep, gens_this in enumerate(epoch_generations):
        ...
        if best_row is None:
            ...
            cur_key = _row_objective_key(row0)
            best_key = _row_objective_key(best_row)
            if cur_key < best_key:
                best_row = dict(row0)
                best_genome = list(genome0)
                best_k = int(row0.get("subset_size", len(idx0)))
        ...
        dynamic_epoch_logs.append(
            {
                ...
                "best_obj_coverage_error": float(row0.get("obj_coverage_error", float("inf"))),
                "best_obj_pinaw": float(row0.get("obj_pinaw", float("inf"))),
                "best_obj_interval_score": float(row0.get("obj_interval_score", float("inf"))),
    ...
    report = {
        ...
        "config": {
            ...
            "temporal_pack_enabled": bool(temporal_pack_enabled),
            ...
            "temporal_pack_cross_quantiles": [float(v) for v in temporal_pack_cross_q],
            "temporal_pack_ratio_eps": float(temporal_pack_ratio_eps),
            "temporal_pack_added_features": list(temporal_pack_added_features),
            "temporal_pack_rolling_added": list(temporal_pack_rolling_added),
            "temporal_pack_momentum_added": list(temporal_pack_momentum_added),
            "temporal_pack_cross_added": list(temporal_pack_cross_added),
            "temporal_pack_ratio_added": list(temporal_pack_ratio_added),
            "inner_opt_enabled": bool(int(args.inner_opt_enabled)),
