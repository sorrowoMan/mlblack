...
from .primitives import PrimitiveRegistry, binary_expr, const_expr, feature_expr, unary_expr
...
def lower_conditional_primitive_specs(
    ...
    *,
    ...
    def safe_step_values(col: np.ndarray, *, cut: float, slope: float) -> np.ndarray:
        z = np.asarray(col, dtype=float).reshape(-1) - float(cut)
        return np.asarray(0.5 * (1.0 + np.tanh(float(slope) * z)), dtype=float)

    def feature_bundle(feature_name: str) -> tuple[int, np.ndarray, dict[str, Any]] | None:
    ...
    for spec in specs:
        ...
        if not spec.source_features:
            continue
        base_bundle = feature_bundle(str(spec.source_features[0]))
        ...
        if multiplier_feature is not None:
            multiplier_bundle = feature_bundle(str(multiplier_feature))
        ...
        if family == "gate_binary":
            ...
            if _binary_like(base_values) and abs(threshold - 0.5) <= 1e-8:
                ...
                step_values = safe_step_values(base_values, cut=threshold, slope=slope)
                expr = binary_expr(
                    "add",
                    const_expr(float(negative_value)),
            ...
            for cat in categories:
                ...
                expr_now = binary_expr("mul", lower_expr, binary_expr("sub", const_expr(1.0), upper_expr))
                lower_values = safe_step_values(base_values, cut=cat_value - 0.5, slope=slope)
                upper_values = safe_step_values(base_values, cut=cat_value + 0.5, slope=slope)
            ...
            values = safe_step_values(base_values, cut=cut, slope=slope)
            ...
            step_values = safe_step_values(base_values, cut=cut, slope=slope)
...
def select_activation_plan(
    *,
    ...
    family_scores: dict[str, float] = {
        ...
        "linear_combo": float(0.50 * interaction_strength + 0.10 * nonlinear_strength),
    ...
    def budget_enabled(fam: str) -> bool:
        if fam not in family_budget_map:
            return True
        return int(family_budget_map[fam]) > 0

    unary_rank = sorted(
        [fam for fam in unary_families_all if float(family_scores.get(fam, 0.0)) > 0.0 and budget_enabled(fam)],
        key=lambda fam: float(family_scores.get(fam, 0.0)),
        reverse=True,
    )
    pair_rank = sorted(
        [fam for fam in pair_families_all if float(family_scores.get(fam, 0.0)) > 0.0 and budget_enabled(fam)],
        key=lambda fam: float(family_scores.get(fam, 0.0)),
        reverse=True,
    )
